...
public class ProductDaoImpl implements ProductDao {
	...
	public Product findProductById(long id){
		...
		String sql = "select id, productname, accnumber, balance, producttype, userid int8 from vk_jp5_products where id = ?";
	...
	public List<Product> getAllProductsByUserId(long userId){
		...
		Connection connection = dataSource.getConnection();
		String sql = "select id, productname, accnumber, balance, producttype, userid int8 from vk_jp5_products where userid = ?";
		PreparedStatement ps = connection.prepareStatement(sql);
		ps.setLong(1, userId);
		ProductType prodTp;
		ResultSet resultSet = ps.executeQuery();
	...
	public void updateProductBalance(long id, double balanceDelta){
		Connection connection = dataSource.getConnection();
		String sql = "update vk_jp5_products set balance = balance + ? where id = ?";
		PreparedStatement ps = connection.prepareStatement(sql);
